...
def POST_rates(file):
    ...
    if file_exists == False:
        return ("File does not exist, please try again." , 404)
        ...
        for i in range(1, row + 1): 
            ...
            if  i != 1:
                ...
                if value_id == str(0):
                    ...
                    add_product = mysql.setData(f'INSERT INTO Rates (`product_id`, `rate`, `scope`) VALUES ("{product_id}", {rate}, "{scope}");')
                                    
                else:

                    update_product = mysql.updateData(f'UPDATE `Rates` SET rate={rate} where product_id="{product_id}" and scope="{scope}";')
